...
public class UsersController extends BaseController {
    ...
    @GetMapping("/avatar")
    @ResponseBody
    public R getAvatar(String token) {

        Log.info("查找当前登录用户的头像Url，token：{}", token);

        String avatar = usersService.getAvatar_Hom(cacheHandle.getUserInfoCache(token));

        return R.successData(avatar);
    }
    ...
    public R addInfo(Users users) {
        ...
        if(usersService.getUserByUserName(users.getUserName()) == null){
            ...
            usersService.addUserAvatar_Hom(users.getId());
