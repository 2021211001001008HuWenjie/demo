...
public class ActiveLogsController extends BaseController {
    ...
    @PostMapping("/add_Hom")
    @ResponseBody
    public R addInfo_Hom(@RequestParam("token")  String token,  @RequestParam("activeId")String activeId ) {

        System.out.println("用户信息：" + token);
        System.out.println("活动id：" + activeId);

        Users user = usersService.getOne(cacheHandle.getUserInfoCache(token));
        ActiveLogs activeLogs = new ActiveLogs();
        activeLogs.setActiveId(activeId);
        if(activeLogsService.isActive(activeId, user.getId())){
            ...
            return R.success("报名成功","200");
        }else{

            return R.success("该活动您已参与，请勿重复报名","201");
        }
    }
    ...
    public R addInfo(String token,  ActiveLogs activeLogs ) {
        ...
        System.out.println("用户信息：" + token);
        System.out.println("ApplyLogs：" + activeLogs.toString());
