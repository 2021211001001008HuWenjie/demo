...
public class ApplyLogsController extends BaseController {
    ...
    public R addInfo(String token, ApplyLogs applyLogs) {
        ...
        if(applyLogsService.isApply(user.getId(), applyLogs.getTeamId(), 1) && applyLogsService.isApply(user.getId(), applyLogs.getTeamId(), 0)){
            ...
            return R.success();
        }else if(applyLogsService.isApply(user.getId(), applyLogs.getTeamId(), 0)){

            return R.warn("您已成功加入，请勿重复操作");
        }else{
            return R.warn("申请审核中，请耐心等待");
        }
    }

    @PostMapping("/add_ById")
    @ResponseBody
    public R addInfo(@RequestParam("token")  String token,  @RequestParam("teamId")String teamId) {
        Users user = usersService.getOne(cacheHandle.getUserInfoCache(token));
        ApplyLogs applyLogs = new ApplyLogs();
        applyLogs.setTeamId(teamId);
        if(applyLogsService.isApply(user.getId(), applyLogs.getTeamId(), 1) && applyLogsService.isApply(user.getId(), applyLogs.getTeamId(), 0)){
            ...
            return R.success("您已成功加入，请勿重复操作",200);
        }else{
            return R.success("申请审核中，请耐心等待",201);
        }
    }
