...
public interface NoticesDao extends BaseMapper<Notices> {
    ...
    @Select("<script>" +
            "SELECT " +
            "id, title, detail, create_time createTime, team_id teamId,imgUrl url " +
            "FROM notices,notices_imgs i " +
            "WHERE team_id IS NULL and id = i.notices_id " +
            "ORDER BY create_time DESC " +
            "</script>")
    public List<Notices_Hom> qrySysNotices();
...
//    public List<Notices> qrySysNotices();
    ...
    @Select("<script>" +
            ...
            "id, title, detail, create_time createTime, team_id teamId " +
            "FROM notices " +
            "WHERE team_id IN (SELECT team_id FROM members WHERE user_id = #{manId}) " +
            "ORDER BY create_time DESC " +
            "</script>")
    ...
    @Select("<script>" +
            ...
            "id, title, detail, create_time createTime, team_id teamId " +
            "FROM notices " +
            "WHERE team_id IN (SELECT team_id FROM members WHERE user_id = #{memId}) " +
            "ORDER BY create_time DESC " +
            "</script>")
